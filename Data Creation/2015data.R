...
year2015 <- read_csv("C:/Users/jorda/OneDrive/Desktop/Thesis/year2015.csv")
...
data2015 <- sapply(teamsab, simplify = FALSE, FUN = function(t) {
  
  away_games <- filter(year2015, away_team == t) %>% 
  ...
  home_games <- filter(year2015, home_team == t) %>%
...
final2015 <- lapply(data2015, function(x){
...
df2015 <- df2015 %>%
  left_join(home2015, by = "SpecID")

df2015 <- df2015 %>%
  left_join(away2015, by = "SpecID")

df2015 <- na.omit(df2015)
df2015$scorediff <- (df2015$home_score - df2015$away_score)

normalize <- function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}

df2015$away_TB <- normalize(df2015$away_TB)
df2015$home_TB <- normalize(df2015$home_TB)

write_csv(df2015, path = "C:/Users/jorda/OneDrive/Desktop/Thesis/df2015",
          col_names = TRUE)