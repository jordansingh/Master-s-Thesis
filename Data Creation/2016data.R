...
year2016 <- read_csv("C:/Users/jorda/OneDrive/Desktop/Thesis/year2016.csv")
...
data2018 <- sapply(teamsab, simplify = FALSE, FUN = function(t) {
  
  away_games <- filter(year2016, away_team == t) %>% 
  ...
  home_games <- filter(year2016, home_team == t) %>%
...
final2016 <- lapply(data2018, function(x){
...
df2016 <- df2016 %>%
  left_join(home2016, by = "SpecID")

df2016 <- df2016 %>%
  left_join(away2016, by = "SpecID")

df2016 <- na.omit(df2016)
df2016$scorediff <- (df2016$home_score - df2016$away_score)

normalize <- function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}

df2016$away_TB <- normalize(df2016$away_TB)
df2016$home_TB <- normalize(df2016$home_TB)

write_csv(df2016, path = "C:/Users/jorda/OneDrive/Desktop/Thesis/df2016",
          col_names = TRUE)