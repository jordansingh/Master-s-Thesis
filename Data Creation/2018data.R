...
year2018 <- read_csv("C:/Users/jorda/OneDrive/Desktop/Thesis/year2018.csv")
...
data2018 <- sapply(teamsab, simplify = FALSE, FUN = function(t) {
  ...
  away_games <- filter(year2018, away_team == t) %>% 
  ...
  home_games <- filter(year2018, home_team == t) %>%
...
final2018 <- lapply(data2018, function(x){
...
df2018 <- df2018 %>%
  left_join(home2018, by = "SpecID")

df2018 <- df2018 %>%
  left_join(away2018, by = "SpecID")

df2018 <- na.omit(df2018)
df2018$scorediff <- (df2018$home_score - df2018$away_score)

normalize <- function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}

df2018$away_TB <- normalize(df2018$away_TB)
df2018$home_TB <- normalize(df2018$home_TB)

write_csv(df2018, path = "C:/Users/jorda/OneDrive/Desktop/Thesis/df2018",
          col_names = TRUE)