...
year2017 <- read_csv("C:/Users/jorda/OneDrive/Desktop/Thesis/year2017.csv")
...
data2017 <- sapply(teamsab, simplify = FALSE, FUN = function(t) {
  
  away_games <- filter(year2017, away_team == t) %>% 
  ...
  home_games <- filter(year2017, home_team == t) %>%
...
final2017 <- lapply(data2017, function(x){
...
df2017 <- df2017 %>%
  left_join(home2017, by = "SpecID")

df2017 <- df2017 %>%
  left_join(away2017, by = "SpecID")

df2017 <- na.omit(df2017)
df2017$scorediff <- (df2017$home_score - df2017$away_score)

normalize <- function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}

df2017$away_TB <- normalize(df2017$away_TB)
df2017$home_TB <- normalize(df2017$home_TB)

write_csv(df2017, path = "C:/Users/jorda/OneDrive/Desktop/Thesis/df2017",
          col_names = TRUE)