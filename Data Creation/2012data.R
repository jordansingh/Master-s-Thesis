...
year2012 <- read_csv("C:/Users/jorda/OneDrive/Desktop/Thesis/year2012.csv")
...
data2012 <- sapply(teamsab, simplify = FALSE, FUN = function(t) {
  
  away_games <- filter(year2012, away_team == t) %>% 
  ...
  home_games <- filter(year2012, home_team == t) %>%
...
final2012 <- lapply(data2012, function(x){
...
df2012 <- df2012 %>%
  left_join(home2012, by = "SpecID")

df2012 <- df2012 %>%
  left_join(away2012, by = "SpecID")

df2012 <- na.omit(df2012)
df2012$scorediff <- (df2012$home_score - df2012$away_score)

normalize <- function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}

df2012$away_TB <- normalize(df2012$away_TB)
df2012$home_TB <- normalize(df2012$home_TB)

write_csv(df2012, path = "C:/Users/jorda/OneDrive/Desktop/Thesis/df2012",
          col_names = TRUE)