...
year2013 <- read_csv("C:/Users/jorda/OneDrive/Desktop/Thesis/year2013.csv")
...
data2013 <- sapply(teamsab, simplify = FALSE, FUN = function(t) {
  
  away_games <- filter(year2013, away_team == t) %>% 
  ...
  home_games <- filter(year2013, home_team == t) %>%
...
final2013 <- lapply(data2013, function(x){
...
df2013 <- df2013 %>%
  left_join(home2013, by = "SpecID")

df2013 <- df2013 %>%
  left_join(away2013, by = "SpecID")

df2013 <- na.omit(df2013)
df2013$scorediff <- (df2013$home_score - df2013$away_score)

normalize <- function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}

df2013$away_TB <- normalize(df2013$away_TB)
df2013$home_TB <- normalize(df2013$home_TB)

write_csv(df2013, path = "C:/Users/jorda/OneDrive/Desktop/Thesis/df2013",
          col_names = TRUE)